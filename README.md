...
- 公開URL: https://a-tozak.github.io/sekiden-kenshu/ （GitHub Pagesは docs/ 配信）
- 形式: 診断型ワークショップ（2026-08-09 全面改訂）。背骨は外﨑の論文「生成AI活用力の発達系統モデル」＝
  第1部で5段階の地図と現在地チェック（15項目・Web上で判定可）、第2部はワークA・B・C、
  第3部はお一人ずつのページ＋お三方共通「学校の仕事」（gakko.html）で「きょうの一歩」→アンケートで終わる。
  本編は「解決したい問い3つ」（AIを使うとは／プロンプトはどう書くか／自分の仕事でどう使うか）の一本道。
  じてん層（jiten/＝Gemini・NotebookLM・Workspace6種・共有権限・プロンプト集）を左メニューのドロワーから常時参照可。
  設計図＝`ワイヤーフレーム_2026-08-09.html`
...
※読みは「せきでん」。2026-08-09に本人指示でリポジトリを sekiden-kenshu に変更（旧 sekita-kenshu は廃止予定）。
